...
int main () {
    ...
    std::thread worker3(framesToTxt, ceil(num_frames/2), floor(3*num_frames/4));
    ...
    std::cout << "Time to process frames to ASCII - using threads (microseconds): " << std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() << '\n';
}
